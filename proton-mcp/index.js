...
process.on('uncaughtException', (err) => {
  process.stderr.write('[proton-mcp] UNCAUGHT EXCEPTION: ' + (err && err.stack ? err.stack : String(err)) + '\n');
  process.exit(1);
});
process.on('unhandledRejection', (reason) => {
  process.stderr.write('[proton-mcp] UNHANDLED REJECTION: ' + (reason && reason.stack ? reason.stack : String(reason)) + '\n');
});
process.stderr.write('[proton-mcp] starting, node=' + process.version + ', platform=' + process.platform + '\n');
...
function loadConfig() {
  ...
  if (process.env.PROTON_BRIDGE_USERNAME && process.env.PROTON_BRIDGE_PASSWORD) {
    // Load signature: use env var if set, otherwise fall back to html signature.txt in project root
    let signature = process.env.PROTON_SIGNATURE || '';
    if (!signature) {
      const sigFile = path.join(__dirname, 'html signature.txt');
      if (fs.existsSync(sigFile)) {
        try {
          signature = fs.readFileSync(sigFile, 'utf-8').trim();
          process.stderr.write('[proton-mcp] loaded signature from html signature.txt\n');
        } catch (err) {
          process.stderr.write(`[proton-mcp] could not read html signature.txt: ${err.message}\n`);
        }
      }
    }
    return {
      ...
      from: process.env.PROTON_BRIDGE_FROM || process.env.PROTON_BRIDGE_USERNAME,
      signature,
    };
  }
...
server.tool(
  ...
  {
    message_id: z.number().describe('Message sequence number from list_messages or list_folder_messages'),
    folder: z.string().optional().describe('Folder the message lives in (default: INBOX). Sequence numbers are scoped to a folder, so use the same folder you got the id from.'),
  },
  async (args) => {
    try {
      const result = await getMessage(getConfig(), args.message_id, args.folder || 'INBOX');
      return { content: [{ type: 'text', text: JSON.stringify(result) }] };
    } catch (err) {
      return errorResult(err.message);
    }
...
server.tool(
  ...
  'Send an email via Proton Mail. Supports CC, BCC, HTML, and file attachments.',
  {
    ...
    from: z.string().optional().describe('Sender address override (defaults to configured bridge_from or bridge_user)'),
  ...
  async (args) => {
    try {
      const cfg = getConfig();
      const result = await sendMessage(cfg, args);
      return { content: [{ type: 'text', text: JSON.stringify(result) }] };
    } catch (err) {
      return errorResult(err.message);
    }
...
server.tool(
  ...
  'Reply to an email, preserving the thread. Set reply_all=true to include all original recipients.',
  {
    ...
    bcc: z.string().optional().describe('BCC recipient(s), comma-separated'),
    folder: z.string().optional().describe('Folder the original message lives in (default: INBOX)'),
    from: z.string().optional().describe('Sender address override (defaults to configured bridge_from or bridge_user)'),
    attachments: z.array(z.object({
      filename: z.string(),
...
server.tool(
  'mail__get_thread',
  'Fetch the full email chain (thread) for a message. Searches INBOX, Sent, Archive, and All Mail.',
  {
    message_id: z.number().describe('Sequence number of any message in the thread'),
    folder: z.string().optional().describe('Folder the seed message lives in (default: INBOX). Sequence numbers are scoped to a folder.'),
  },
  async (args) => {
    try {
      const result = await getThread(getConfig(), args.message_id, args.folder || 'INBOX');
      return { content: [{ type: 'text', text: JSON.stringify(result) }] };
    } catch (err) {
      return errorResult(err.message);
    }
...
server.tool(
  ...
  'Mark an email as read or unread',
  {
    ...
    folder: z.string().optional().describe('Folder the message lives in (default: INBOX)'),
  },
  async (args) => {
    try {
      const result = await markMessage(getConfig(), args.message_id, args.read, args.folder || 'INBOX');
      return { content: [{ type: 'text', text: JSON.stringify(result) }] };
    } catch (err) {
      return errorResult(err.message);
    }
...
server.tool(
  ...
  async (args) => {
    try {
      const result = await forwardMessage(getConfig(), {
        originalMessageId: args.message_id,
        to: args.to,
        body: args.body,
        cc: args.cc,
...
server.tool(
  ...
  async (args) => {
    try {
      return { content: [{ type: 'text', text: JSON.stringify(await starMessage(getConfig(), args.message_id, args.star, args.folder || 'INBOX')) }] };
...
server.tool(
  ...
  'Permanently delete an email from the given folder',
  {
    message_id: z.number().describe('Message sequence number'),
    folder: z.string().optional().describe('Folder the message lives in (default: INBOX)'),
  },
  async (args) => {
    try {
      return { content: [{ type: 'text', text: JSON.stringify(await deleteMessage(getConfig(), args.message_id, args.folder || 'INBOX')) }] };
    } catch (err) { return errorResult(err.message); }
  },
);
...
server.tool(
  ...
  {
    message_id: z.number().describe('Message sequence number (scoped to the source folder)'),
    destination: z.string().describe('Destination folder name (use mail__list_folders to see available folders)'),
    folder: z.string().optional().describe('Source folder the message lives in (default: INBOX)'),
  },
  async (args) => {
    try {
      return { content: [{ type: 'text', text: JSON.stringify(await moveMessage(getConfig(), args.message_id, args.destination, args.folder || 'INBOX')) }] };
    } catch (err) { return errorResult(err.message); }
  },
);
...
server.tool(
  ...
  async (args) => {
    try {
      return { content: [{ type: 'text', text: JSON.stringify(await getAttachments(getConfig(), args.message_id, args.folder || 'INBOX')) }] };
    } catch (err) { return errorResult(err.message); }
  },
);
...
server.tool(
  ...
  {
    vault: z.string().optional().describe('Vault name (default: Personal)'),
  },
  async (args) => {
    try {
...
server.tool(
  ...
  async (args) => {
    try {
      ...
      // pass-cli requires a subcommand: `random` or `passphrase`.
      const genArgs = ['password', 'generate', 'random'];
      if (args.length) genArgs.push('--length', `${args.length}`);
      // Do NOT force PROTON_PASS_KEY_PROVIDER here — pass-cli's login decides
      // which key provider to use (e.g. wincred on Windows). Forcing 'fs' will
      // make it look for a file-based key that doesn't exist and produce a
      // misleading "database corrupted" error.
...
process.stderr.write('[proton-mcp] connecting transport...\n');
try {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  process.stderr.write('[proton-mcp] transport connected OK\n');
} catch (err) {
  process.stderr.write('[proton-mcp] transport connect FAILED: ' + (err && err.stack ? err.stack : String(err)) + '\n');
  process.exit(1);
}
